...
if(transcript_ids != "all"):
    ...
    if (fwrd_allsamp_hist_fil != ""):
        for filename in fwrd_allsamp_hist_fil:
            ...
            fwrd_fil.append(df.loc[:, df.columns.isin(['sample_Name', 'Freq_forward', 'bins_foward'])])
    ...
    if (rev_allsamp_hist_fil != ""):
        for filename in rev_allsamp_hist_fil:
            df = pd.read_csv(filename, index_col = None, header = 0, sep = "\t")
            rev_fil.append(df.loc[:, df.columns.isin(['sample_Name', 'Freq_rev', 'bins_rev'])])
...
if(transcript_ids != "all"):
    if not fwrd_allsamp_hist_fil.empty:
        ...
        hist_fwrd_full = alt.Chart(fwrd_allsamp_hist_fil).mark_line(interpolate = 'step-after').encode(x = alt.X('bins_foward', 
        ...
        cht_rd_len_fwd_full = alt.Chart(fwrd_allsamp_hist_fil).mark_rule(color = 'black',strokeDash = [3,5]).encode(
            x = 'read_length')
        ...
        hist_rev_full = alt.Chart(rev_allsamp_hist_fil).mark_line(interpolate = 'step-after').encode(x = alt.X('bins_rev', 
        ...
        cht_rd_len_rev = alt.Chart(rev_allsamp_hist_fil).mark_rule(color = 'black',strokeDash = [3,5]).encode(
        x = 'read_length')
