...
if ((fgsea_res %>% count() %>% pull(n)) == 0) {
    ...
    annotated <- fgsea_res %>%
        unnest(leadingEdge) %>%
        mutate(
            leading_edge_entrez_id = mapIds(
                leadingEdge,
                x = get(snakemake@params[["bioc_species_pkg"]]),
                keytype = "SYMBOL",
                column = "ENTREZID"
            ),
            leading_edge_ens_gene = mapIds(
                leadingEdge,
                x = get(snakemake@params[["bioc_species_pkg"]]),
                keytype = "SYMBOL",
                column = "ENSEMBL"
        ...
        inner_join(
            fgsea_res %>% dplyr::select(-leadingEdge),
            by = "pathway"
        ) %>%
        dplyr::relocate(
            c(
...
ggsave(
    filename = snakemake@output[["plot_collapsed"]],
    plot = tg,
    width = 15,
    height = height,
    limitsize = FALSE
