...
if aligned_reads['Transcript'].str.contains(transcript_ids).any():
    ...
    Freq_fwrd_fil, bins_fwrd_fil = np.histogram(fwrd_filtered_transcript_data[sample_name + '_forward_strand'], 
        bins = read_length, range = [0,max(fwrd_filtered_transcript_data['LN'])])
    ...
    Freq_fwrd, bins_fwrd = np.histogram(aligned_reads[sample_name + '_forward_strand'], 
        bins = read_length, range = [0,max(aligned_reads['LN'])])
    Freq_fwrd_trim, bins_fwrd_trim = np.histogram(forward_strand[sample_name + '_forward_strand'], 
        bins =read_length, range = [0, 20000])
...
if read_min['Transcript'].str.contains(transcript_ids).any():
    ...
    Freq_rev_fil, bins_rev_fil = np.histogram(rev_filtered_transcript_data['Start'], 
        bins = read_length, range = [0,max(rev_filtered_transcript_data['LN'])])
